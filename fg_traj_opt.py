...
def visualize_factor_graph(graph, save_path=None):
    G = nx.DiGraph()
    for i in range(graph.size()):
        ...
        if len(keys) > 1:
            for j in range(len(keys) - 1):
                G.add_edge(gtsam.DefaultKeyFormatter(keys[j]), gtsam.DefaultKeyFormatter(keys[j + 1]))

    plt.figure(figsize=(15, 10))  # Increased figure size for better readability
    pos = nx.kamada_kawai_layout(G)  # Use a better layout to reduce clutter and provide clearer separation
    nx.draw(G, pos, with_labels=True, node_size=800, node_color='lightblue', font_size=10, font_weight='bold', edge_color='gray')
    plt.title("Improved Factor Graph Diagram")
    if save_path:
...
def main(start, goal, T):
    ...
    visualize_factor_graph(graph, save_path="images/fg_traj_opt/factor_graph.png")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("images/fg_traj_opt/trajectory.png")
    plt.show()

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Trajectory Optimization via Factor Graphs')
    ...
    main(args.start, args.goal, args.T)
